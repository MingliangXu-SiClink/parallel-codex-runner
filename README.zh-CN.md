...
默认情况下，PCR 会先启动 4 个候选 Agent，再由 2 个相互隔离的综合 Agent 审核所有成功候选，并在各自工作区中取长补短。每个结果仍是一条完整分支，你可以检查并采用任意成功的候选或综合结果。使用 `--synthesis-agents 0` 可以跳过第二阶段。
...
5. 根据配置启动相互隔离的 Agent 审核并综合候选结果；
6. 推荐一个成功结果，同时把最终选择权留给你。
...
排队提问等待时间、是否回写、是否保留工作区以及当前选择的 resume 会话。配置保存在
用户的 PCR 状态目录中，不会写入项目本身；如果启动命令显式传入这些选项，则以命令行
选项为准。
...
PCR 默认启动 2 个综合 Agent。可以直接修改顶部的 `SYNTHESIS_AGENTS`，也可以在提交下一条需求前调整：
...
| `/followupdelay <seconds>` | 设置排队提问自动开始前的确认时间。 |
| `/synthesis <n\|off>` | 设置尚未开始的综合阶段 Agent 数量。 |

### 管理排队中的后续提问

存在排队提问时，输入 `/queue` 即可打开编辑器。你可以从列表中选择一条提问，修改完整内容，使用箭头按钮调整执行顺序，或者将其移除。`APPLY` 会一次性应用修改；`CANCEL` 或 `Esc` 会放弃本次修改。编辑器打开时自动续接会暂停；关闭后，如果队列已经可以执行，会重新给出完整的确认时间。

顶部的 `FOLLOW_UP_DELAY` 控制这段确认时间，单位为秒，默认值为 `60`。Agent
运行期间或倒计时进行中都可以直接修改；也可以输入 `/followupdelay 15`，或在启动时
传入 `--follow-up-delay 15`。设置为 `0` 时，排队提问不再等待。确认期间切换 Agent
会从当前显示的 Agent 重新开始完整倒计时。
...
一次性模式默认运行 4 个候选和 2 个综合 Agent，自动推荐一个成功结果，将它同步到原始工作区，然后清理临时工作区。
...
正常采用结果或退出后，PCR 会删除普通候选与综合 Agent 的工作区；启用 `--keep-workspaces` 时除外。运行日志、元数据和可恢复的会话状态会继续保留在本轮目录中。
...
| `--follow-up-delay` | 排队提问从当前 Agent 自动开始前的 TUI 确认时间，默认 `60` 秒；设为 `0` 可立即开始。 |
...
| `/followupdelay <seconds>` | 设置排队提问的确认时间；设为 `0` 可立即开始。 |
| `/synthesis <n\|off>` | 设置尚未开始的综合阶段 Agent 数量。 |
...
| 路径 | 内容 |
| --- | --- |
| `prompt.txt` | 发送给普通候选和综合 Agent 的原始需求。 |
| `synthesis_context.md` | 原始需求，以及成功候选工作区和回复文件的路径。 |
| `synthesis_instructions.txt` | 注入第二阶段综合 Agent 的内部 developer instructions。 |
