...
默认情况下，每个 Agent 都代表一条完整分支，最终采用其中一条。对于重要任务，你也可以启用可选的第二阶段：让一组全新的综合 Agent 审核所有成功候选，在各自工作区中取长补短，再生成新的可选分支。
...
可以直接修改顶部的 `SYNTHESIS_AGENTS`，也可以在提交下一条需求前输入：

```text
/synthesis 3
```

第一阶段的候选全部结束后，PCR 会启动 3 个全新的综合 Agent。它们各自获得一份干净的原始工作区副本，以及所有成功候选工作区和最终回复的路径；提示词会明确要求它们只参考、不修改这些来源。对于代码修改任务，综合 Agent 会比较实际实现，在自己的工作区中整合兼容的优点并运行验证；对于回答型任务，它会消除候选回答中的冲突，给出一份完整答案。

只要有综合 Agent 成功，`RECOMMEND_BY` 就会优先在这些结果中推荐；如果全部失败，则回退到第一阶段的成功候选。这只影响推荐，不限制你的选择：两个阶段中的任意成功 Agent 都可以继续对话或最终采用。`/more <n>` 的含义仍然不同，它只是为当前问题增加普通的独立候选。
...
# 先运行 6 个候选，再运行 2 个相互独立的综合 Agent
...
| `--synthesis-agents` | 候选完成后启动的全新审核与综合 Agent 数量，默认 `0`。 |
...
| 路径 | 内容 |
| --- | --- |
| `prompt.txt` | 发送给第一阶段候选的原始需求。 |
| `synthesis_context.md` | 原始需求，以及成功候选工作区和回复文件的路径。 |
| `synthesis_prompt.txt` | 发送给第二阶段综合 Agent 的审核与整合说明。 |
