...
Candidate workspaces are retained automatically while the run awaits review,
subject to the configured TTL and retention period. Leave `keep_workspaces=false`
unless the user explicitly wants those workspaces to remain after accepting a
result; it is not needed for the review step.
...
- Use `pcr_kill_agent` to stop one running candidate while allowing the others to continue. Queued candidates are not killable and start normally.
