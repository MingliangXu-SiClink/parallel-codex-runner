...
class Input(ScrollView):
        ...
        if not self.value:
            ...
            if self.has_focus:
                ...
                if self._cursor_visible:
                    ...
                    cursor_end = (
                        cursor + 1
                        if self.cursor_at_end or self.password
                        else next_grapheme_boundary(value, cursor)
                    )
                    result.stylize(cursor_style, cursor, cursor_end)
    ...
    def _cell_offset_to_index(self, offset: int) -> int:
        ...
        scroll_x, _ = self.scroll_offset
        offset += scroll_x
        return cell_width_to_column_index(self.value, offset, 4)
    ...
    def action_cursor_left(self, select: bool = False) -> None:
        ...
        if select:
            self.selection = Selection(
                start, previous_grapheme_boundary(self.value, end)
            )
        else:
            if self.selection.is_empty:
                self.cursor_position = previous_grapheme_boundary(
    ...
    def action_cursor_right(self, select: bool = False) -> None:
        ...
        start, end = self.selection
        if select:
            self.selection = Selection(start, next_grapheme_boundary(self.value, end))
        else:
    ...
    def action_cursor_left_word(self, select: bool = False) -> None:
        ...
        if self.password:
            ...
            target = input_word_left(self.value, self.cursor_position)
    ...
    def action_cursor_right_word(self, select: bool = False) -> None:
        ...
        if self.password:
            ...
            start, end = self.selection
            target = input_word_right(self.value, self.cursor_position)
    ...
    def action_delete_right(self) -> None:
        ...
        if self.selection.is_empty:
            self.delete(
                ...
                next_grapheme_boundary(self.value, self.cursor_position),
            )
        else:
    ...
    def action_delete_right_word(self) -> None:
        ...
        if self.password:
            ...
            self.delete(
                self.cursor_position,
                input_word_right(self.value, self.cursor_position),
            )
    ...
    def action_delete_left(self) -> None:
        ...
        if self.selection.is_empty:
            self.delete(
                previous_grapheme_boundary(self.value, self.cursor_position),
                self.cursor_position,
            )
        else:
            self.delete_selection()
    ...
    def action_delete_left_word(self) -> None:
        ...
        if self.password:
            ...
            self.delete(
                input_word_left(self.value, self.cursor_position),
                self.cursor_position,
            )
