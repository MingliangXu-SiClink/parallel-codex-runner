...
from textual._unicode import next_grapheme_boundary, previous_grapheme_boundary
...
class DocumentNavigator:
    ...
    def get_location_left(self, location: Location) -> Location:
        ...
        target_column = (
            previous_grapheme_boundary(self._document[row], column)
            if column != 0
            else length_of_row_above
        )
    ...
    def get_location_right(self, location: Location) -> Location:
        ...
        target_column = (
            0
            if is_end_of_line
            else next_grapheme_boundary(self._document[row], column)
        )
        return target_row, target_column
