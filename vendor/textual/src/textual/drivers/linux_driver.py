...
def _get_kitty_protocol_flags() -> int:
    """Return keyboard protocol flags compatible with the current terminal."""
    if (
        os.environ.get("LC_TERMINAL") == "iTerm2"
        or os.environ.get("TERM_PROGRAM") == "iTerm.app"
    ):
        # iTerm 3.6.11 loses IME commits when both report-all and associated-text
        # modes are enabled. Disambiguation still preserves modified keys.
        return KITTY_DISAMBIGUATE_ESCAPE_CODES
    return (
...
class LinuxDriver(Driver):
    ...
    def start_application_mode(self):
        ...
        if not constants.DISABLE_KITTY_KEY:
            ...
            self.write(f"\x1b[>{_get_kitty_protocol_flags()}u")
